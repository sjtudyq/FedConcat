...
def train_net_encoder_classifier(net_id, net_encoder, net_classifier, train_dataloader, test_dataloader, epochs, lr, args_optimizer, calc_loss=False, device="cpu"):
    logger.info('Training network %s' % str(net_id))
...
def local_train_net_encoder_classifier(encoder_list, classifier_list, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
...
def IFCA(encoder_global_para, classifier_global_para, encoder_list, classifier_list, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id in selected:
        ...
        for i in range(len(encoder_global_para)):
            encoder_list[net_id].load_state_dict(encoder_global_para[i])
            classifier_list[net_id].load_state_dict(classifier_global_para[i])

            encoder_list[net_id].to(device)
            classifier_list[net_id].to(device)

            loss = train_net_encoder_classifier(net_id, encoder_list[net_id], classifier_list[net_id], train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, calc_loss=True, device=device)
            if loss < min_loss:
                min_loss = loss
                cluster_id = i
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.encoder_round):
            ...
            group_assign = IFCA(encoder_global_para, classifier_global_para, encoder_list, classifier_list, participation, args, net_dataidx_map, test_dl = test_dl_global, device=device)
