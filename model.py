...
class SimpleAE_Cifar(nn.Module):
    def __init__(self):
        super(SimpleAE_Cifar, self).__init__()
        ...
        self.linear1 = nn.Linear(32*6*6, 32*32*2)
        self.linear2 = nn.Linear(32*32*2, 32*32*3)


    def forward(self, x):
        encoded = self.encoder(x).view(-1, 32*6*6)
        decoded = F.relu(self.linear1(encoded))
        decoded = self.linear2(decoded)
...
class ModerateEncoder(nn.Module):
    def __init__(self, output_dim=10):
        super(ModerateEncoder, self).__init__()
        self.conv_layer = nn.Sequential(
            # Conv Layer block 1
            nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
