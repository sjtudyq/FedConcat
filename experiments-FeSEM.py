...
def train_net_encoder_classifier(net_id, net_encoder, net_classifier, global_encoder, global_classifier, train_dataloader, test_dataloader, epochs, lr, args_optimizer, calc_loss=False, device="cpu"):
    logger.info('Training network %s' % str(net_id))
    ...
    for epoch in range(epochs):
        ...
        for tmp in train_dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                fed_prox_reg = 0
                for param_index in global_encoder.keys():
                    param = net_encoder.state_dict()[param_index]
                    fed_prox_reg += ((mu / 2) * torch.norm((param - global_encoder[param_index]))**2)

                for param_index in global_classifier.keys():
                    param = net_classifier.state_dict()[param_index]
                    fed_prox_reg += ((mu / 2) * torch.norm((param - global_classifier[param_index]))**2)
...
def local_train_net_encoder_classifier(encoder_list, classifier_list, global_encoder, global_classifier, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id in range(len(encoder_list)):
        ...
        train_net_encoder_classifier(net_id, encoder_list[net_id], classifier_list[net_id], global_encoder, global_classifier, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, device=device)

        encoder_list[net_id].to("cpu")
        classifier_list[net_id].to("cpu")

def FeSEM(encoder_global_para, classifier_global_para, encoder_list, classifier_list, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    group_assign = [[] for i in range(len(encoder_global_para))]
    ...
    for net_id in selected:
        ...
        for i in range(len(encoder_global_para)):
            global_encoder = encoder_global_para[i]
            global_classifier = classifier_global_para[i]

            fed_prox_reg = 0
            for param_index in global_encoder.keys():
                param = net_encoder.state_dict()[param_index]
                fed_prox_reg += torch.norm((param - global_encoder[param_index]))**2

            for param_index in global_classifier.keys():
                param = net_classifier.state_dict()[param_index]
                fed_prox_reg += torch.norm((param - global_classifier[param_index]))**2

            #logger.info(fed_prox_reg)
            if fed_prox_reg < min_loss:
                min_loss = fed_prox_reg
                cluster_id = i

            encoder_list[net_id].to("cpu")
            classifier_list[net_id].to("cpu")
        group_assign[cluster_id].append(net_id)
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.encoder_round):
            ...
            if round == 0:
                group_assign = np.split(participation, num_K)
            else:
                group_assign = FeSEM(encoder_global_para, classifier_global_para, encoder_list, classifier_list, participation, args, net_dataidx_map, test_dl = test_dl_global, device=device)

            for i in range(num_K):
                selected = group_assign[i]
                if len(selected) == 0:
                    continue
                for idx in selected:
                ...
                local_train_net_encoder_classifier(encoder_list, classifier_list, encoder_global_para[i], classifier_global_para[i], selected, args, net_dataidx_map, test_dl = test_dl_global, device=device)
