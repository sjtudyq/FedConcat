...
def train_net_encoder_classifier(group_assign, net_id, net_encoder, net_classifier, global_encoder, global_classifier, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, calc_loss=False, device="cpu"):
    logger.info('Training network %s' % str(net_id))
        ...
        for tmp in train_dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                if calc_loss:
                    ...
                    fed_prox_reg = 0
                    for i in range(len(global_encoder)):
                        for param_index in global_encoder[i].keys():
                            param = net_encoder.state_dict()[param_index]
                            fed_prox_reg += (group_assign[i][net_id] * (mu / 2) * torch.norm((param - global_encoder[i][param_index]))**2)

                        for param_index in global_classifier[i].keys():
                            param = net_classifier.state_dict()[param_index]
                            fed_prox_reg += (group_assign[i][net_id] * (mu / 2) * torch.norm((param - global_classifier[i][param_index]))**2)
    ...
    if calc_loss:
        return epoch_loss_collector



def local_train_net_encoder_classifier(group_assign, encoder_list, classifier_list, global_encoder, global_classifier, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id in range(len(encoder_list)):
        ...
        train_net_encoder_classifier(group_assign, net_id, encoder_list[net_id], classifier_list[net_id], global_encoder, global_classifier, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args, device=device)

        encoder_list[net_id].to("cpu")
        classifier_list[net_id].to("cpu")

def FedSoft(group_assign, encoder_global_para, classifier_global_para, encoder_list, classifier_list, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id in range(args.n_parties):
        ...
        if args.noise_type == 'space':
            train_dl_local, test_dl_local, _, _ = get_dataloader(args.dataset, args.datadir, 1, 1, dataidxs, noise_level, net_id, args.n_parties-1)
        else:
            noise_level = args.noise / (args.n_parties - 1) * net_id
            train_dl_local, test_dl_local, _, _ = get_dataloader(args.dataset, args.datadir, 1, 1, dataidxs, noise_level)
        n_epoch = 1

        loss_all = []
        for i in range(len(encoder_global_para)):
            encoder.load_state_dict(encoder_global_para[i])
            classifier.load_state_dict(classifier_global_para[i])

            encoder.to(device)
            classifier.to(device)

            loss = train_net_encoder_classifier(None, net_id, encoder, classifier, None, None, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args, calc_loss=True, device=device)
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        encoder_global_para = [encoder_list[i].state_dict() for i in range(num_K)]
        classifier_global_para = [classifier_list[i].state_dict() for i in range(num_K)]
        ...
        logger.info(group)
        '''
        group_assign = [[1/num_K for i in range(args.n_parties)] for j in range(num_K)]
        for round in range(args.encoder_round):
            logger.info("in comm round:" + str(round))
            ...
            local_train_net_encoder_classifier(group_assign, encoder_list, classifier_list, encoder_global_para, classifier_global_para, selected, args, net_dataidx_map, test_dl = test_dl_global, device=device)
