...
def compute_accuracy_ae(model_list, dataloader, cate_loss, get_confusion_matrix=False, device="cpu"):
        ...
        for tmp in dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                for i in range(10):
                    model = model_list[i]
                    model.to(device)
                    re = model(x).reshape(x.shape)
                    loss = loss_function(re, x)
                    loss = torch.mean(loss, dim = (1, 2, 3)) / cate_loss[i]
                    out.append(loss.tolist())
                out = torch.Tensor(out)
                logger.info("out: "+str(out))
                pred_label = torch.min(out, dim = 0)[1]
                #logger.info(pred_label)

                total += x.data.size()[0]
                correct += (pred_label == target.data).sum().item()
                logger.info("predict: "+str(pred_label))
                logger.info("correct: "+str(target.data))
